...
func NewCamera() Camera {
	...
	camera.LowerLeftCorner = Sum(Sum(Sum(camera.Origin, Div(camera.Horizontal, 2)), Div(camera.Vertical, 2)), NewVec3(0, 0, focalLength))
...
func (c Camera) GetRay(u, v float64) Ray {
	...
	return NewRay(c.Origin, Sum(Add(c.LowerLeftCorner, Add(MulF(c.Horizontal, u), MulF(c.Vertical, v))), c.Origin))
}
